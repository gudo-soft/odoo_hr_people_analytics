odoo.define('ht.people_analytics.pa_widget', function (require) {
"use strict";
...
var TimeCounter = common.AbstractField.extend(common.ReinitializeFieldMixin, {
    ...
    start_time_counter: function(){
        ...
        if (this.field_manager.datarecord.state == 'progress') {
              //console.log('if');
            this.real_duration += 1000;
            ...
            clearTimeout(this.timer);
             ...
             //console.log('render');
        this.$el.html($('<span>' + moment.utc(this.real_duration).format("HH:mm:ss") + '</span>'));
    },
    render_value: function() {
        ...
        var hr_activity_record_line = new Model('hr.activity.record.line');
            hr_activity_record_line.query(['date_start' , 'date_finished' , 'duration' , 'state'])
               .filter([['activity_record_id', '=', this.field_manager.datarecord.id]])
       ...
       // new Model('hr.activity_record_line').call('search_read', [hr_activity_record_line_domain, []]).then(function(result) {
            ...
            if (self.get("effective_readonly")) {
                ...
                _.each(result, function(data) {
                    ...
                    self.real_duration += data.date_finished ? self.get_date_difference(data.date_start, data.date_finished) : self.get_date_difference(time.auto_str_to_date(data.date_start), current_date);
...
core.form_widget_registry.add('pa_time_counter', TimeCounter);
